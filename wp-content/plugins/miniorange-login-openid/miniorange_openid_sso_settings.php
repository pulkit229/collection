...
require('view/link_social_account/mo_openid_link_soc_acc_funct.php');
...
Thank you.';
    ...
    function new_miniorange_openid_menu() {
        ...
        $page = add_menu_page( 'MO OpenID Settings ' . __( 'Configure OpenID', 'mo_openid_settings' ), 'miniOrange Social Login, Sharing', 'administrator',
            'mo_openid_settings', 'mo_register_openid' ,plugin_dir_url(__FILE__) . 'includes/images/miniorange_icon.png');
            ...
            case 'mo_openid_account_linking':
                {
                    if (!mo_openid_restrict_user()) {
                        ...
                        if (!wp_verify_nonce($nonce, 'mo-openid-enable-account-linking-nonce')) {
                            ...
                            update_option('mo_account_linking_new_user_button', isset($_POST['mo_account_linking_new_user_button']) ? sanitize_text_field($_POST['mo_account_linking_new_user_button']) : "");
                            update_option('mo_account_linking_existing_user_button', isset($_POST['mo_account_linking_existing_user_button']) ? sanitize_text_field($_POST['mo_account_linking_existing_user_button']) : "");
                            update_option('mo_account_linking_new_user_instruction', isset($_POST['mo_account_linking_new_user_instruction']) ? sanitize_text_field($_POST['mo_account_linking_new_user_instruction']) : "");
                            update_option('mo_account_linking_existing_user_instruction', isset($_POST['mo_account_linking_existing_user_instruction']) ? sanitize_text_field($_POST['mo_account_linking_existing_user_instruction']) : "");
                            update_option('mo_account_linking_extra_instruction', isset($_POST['mo_account_linking_extra_instruction']) ? sanitize_text_field($_POST['mo_account_linking_extra_instruction']) : "");
                            update_option('moopenid_logo_check_account', isset($_POST['moopenid_logo_check_account']) ? sanitize_text_field($_POST['moopenid_logo_check_account']) : 0);
                            update_option('mo_openid_message', 'Your settings are saved successfully.');
                            mo_openid_show_success_message();
                        }
                    }
