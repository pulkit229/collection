...
function mo_openid_process_social_login(){
    ...
    $appuserdetails = array(
        ...
        'last_name' => isset($_POST['lastName']) ? mo_openid_decrypt_sanitize($_POST['lastName']): '',
        'email' => $decrypted_email,
        'user_name' => $decrypted_user_name,
        'user_url' => isset($_POST['profileUrl']) ? mo_openid_decrypt_sanitize($_POST['profileUrl']): '',
        'user_picture' => isset($_POST['profilePic']) ? mo_openid_decrypt_sanitize($_POST['profilePic']): '',
        'social_user_id' => isset($_POST['userid']) ? mo_openid_decrypt_sanitize($_POST['userid']): '',
    );
...
function mo_openid_process_user_details($appuserdetails,$appname)
{
    ...
    if(get_option('mo_openid_auto_register_enable')) {
        if (get_option('mo_openid_account_linking_enable')) {
            echo mo_openid_account_linking_form($user_name, $user_email, $first_name, $last_name, $user_full_name, $user_url, $user_picture, $appname, $social_user_id);
            exit;
        }
        else if (get_option('mo_openid_enable_profile_completion') && ($session_values['user_email'] == '' || $session_values['username'] == '')) { // if newa user and profile completion is enabled
            echo mo_openid_profile_completion_form($session_values);
            exit;
        }
        else {
